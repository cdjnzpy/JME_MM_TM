...
class Trainer():
    ...
    def train(self):
        ...
        for _ in range(self.args.draw_sample_test):
            data_test = self.Data_set.draw_data(self.args.T_period_test) #128*(1y 1w 2eps)
            w_grids_t = data_test[:,1]
            sample_test_combine.append([data_test, w_grids_t])

            data_test_numpy = data_test.numpy().tolist()
            sample_save.append(data_test_numpy)

        sample_save = np.array(sample_save)
        np.save(self.args.save_test_data_path, sample_save)
        ...
        for i_epoch in range(self.args.train_epoch):
            ...
            #输出w维度和 初始收入冲击
            w_grids = data_train[:,1].unsqueeze(1) #128*1
            ...
            c_w_h = Phi_h_Model(model_state)

            c_w_ratio = 1/(1+torch.exp(-c_w_h[:,0].unsqueeze(1)))
            h = torch.exp(c_w_h[:,1].unsqueeze(1))
            ...
            remind_next_1 = self.get_shock_change(Phi_h_Model, shock_eps_1, c_now, w_grids, h)

            #shock_2
            shock_eps_2 = data_train[:,1+self.args.draw_grid_each_epoch+1].unsqueeze(1)

            remind_next_2 = self.get_shock_change(Phi_h_Model, shock_eps_2, c_now, w_grids, h)

            #计算欧拉剩余
            loss = torch.mean(torch.square(FB_now) +(remind_next_1 * remind_next_2))
            ...
            if i_epoch%500 == 0:
                self.plot_policy(Phi_h_Model, i_epoch)
    ...
    def plot_policy(self, Model, i_epoch):
        #输出policy
        ws = []
        cs = []
        for i in range(400):
        ...
        plt.xlabel("w grids")
        plt.ylabel("c grids")
        plt.savefig(self.args.save_learning_pho+str(i_epoch)+"_pho.png")

    def get_shock_change(self, Model, shock, c_now, w_last, h):
        #shock_1
        ...
        c_w_h_next = Model(model_state_next)

        c_w_ratio_next = 1/(1+torch.exp(-c_w_h_next[:,0].unsqueeze(1)))
        h_next = torch.exp(c_w_h_next[:,1].unsqueeze(1))
        
        c_next = c_w_ratio_next * w_next

        remind_next = self.args.rate * self.args.beta * (c_next/c_now)**(-self.args.gamma) - h
    ...
    def eval_model(self, Model, sample_test_combine):
        ...
        for i_test in range(self.args.draw_sample_test):
            ...
            #输出w维度和 初始收入冲击
            w_grids_test = w_grids_t.unsqueeze(1) #128*1
            ...
            for t_period in range(self.args.T_period):
                ...
                c_w_h= Model(model_state)  #128*2 to 128*1
                c_w_ratio = 1/(1+torch.exp(-c_w_h[:,0].unsqueeze(1)))
                h = torch.exp(c_w_h[:,1].unsqueeze(1))
